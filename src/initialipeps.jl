...
function initialfromD(model, folder, Ni, Nj, symmetry, 
    ...
    newD = sum(newdimsD)
    newχ = sum(newdimsχ)
    oldipeps, key = init_ipeps(model; Ni=Ni, Nj=Nj, symmetry=symmetry, atype=Array, folder=folder, D=oldD, χ=oldχ, indD = oldindD, indχ = oldindχ, dimsD = olddimsD, dimsχ = olddimsχ, tol=1e-10, maxiter=10)
    ...
    newinfo = zerosinitial(Val(symmetry), Array, ComplexF64, newD, newD, 4, newD, newD; 
                    dir = [-1, -1, 1, 1, 1], 
